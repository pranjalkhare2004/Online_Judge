...
// Instance method to update streak when user solves a problem
UserSchema.methods.updateStreak = function() {
  ...
  if (lastSolved) {
    lastSolved.setHours(0, 0, 0, 0); // Start of last solved day
  }
  ...
  if (!lastSolved) {
    ...
    // Already solved a problem today - no streak change
    return;
  } else if (today.getTime() - lastSolved.getTime() === oneDay) {
    // Consecutive day - increment streak
    this.currentStreak = (this.currentStreak || 0) + 1;
    this.longestStreak = Math.max(this.longestStreak || 0, this.currentStreak);
    this.lastSolvedDate = today;
  } else if (today.getTime() - lastSolved.getTime() > oneDay) {
    // Gap in solving - reset current streak
    this.currentStreak = 1;
    this.lastSolvedDate = today;
  }
  // If solving from future date, just update last solved date
  else if (today.getTime() > lastSolved.getTime()) {
    this.lastSolvedDate = today;
  }
};
