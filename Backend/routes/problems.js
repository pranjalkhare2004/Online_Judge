...
const { validateProblemCreation, validateSubmission, validateProblemSubmission } = require('../middleware/validation');
    ...
    try {
      ...
      if (identifier.match(/^[0-9a-fA-F]{24}$/)) {
        problem = await Problem.findOne({ _id: identifier, isActive: true })
          .select('title slug difficulty description constraints examples tags timeLimit memoryLimit testCases')
          .populate('testCases', 'input expectedOutput isPublic timeLimit memoryLimit')
          .lean();
      }
    } catch (error) {
...
}), validateProblemSubmission, async (req, res) => {
  try {
