...
const EnhancedWebSocketService = require('./services/websocketService');
...
async function startServer() {
  try {
    ...
    // Store WebSocket service instance for potential use in routes
    app.set('wsService', wsService);

    // Add WebSocket stats to health check
    const originalHealthHandler = app._router.stack.find(layer => 
      layer.route && layer.route.path === '/api/health'
    );
    
    // Enhanced health check with WebSocket stats
    app.get('/api/health/detailed', async (req, res) => {
      try {
        ...
        res.json({
          status: 'OK',
          message: 'Online Judge Backend is running with enhanced features',
          timestamp: new Date().toISOString(),
          environment: process.env.NODE_ENV || 'development',
          version: process.env.npm_package_version || '1.0.0',
          database: {
