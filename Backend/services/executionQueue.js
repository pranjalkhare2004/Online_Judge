...
const { executeCodeSecure, isDockerAvailable } = require('./dockerCompiler');
...
/**
 * Execute code directly (fallback when queue is not available)
 */
async function executeCodeDirectly(code, language, testCases) {
  const dockerAvailable = await isDockerAvailable();
  
  if (dockerAvailable) {
    console.log('🐳 Using Docker execution');
    return await executeCodeSecure(code, language, testCases);
  } else {
    console.log('🏠 Using local execution');
