  ...
  console.log(`[DEBUG] Docker path: ${dockerSandboxPath}`);
        ...
        const timeout = setTimeout(async () => {
          try {
            ...
            console.log(`[DEBUG] Container killed due to timeout`);
          } catch (killError) {
            console.warn('Container kill warning (likely already finished):', killError.message);
          }
        }, 5000); // 5s timeout
      ...
      console.log(`[DEBUG] Parsed stderr: "${stderr}"`);
        ...
        const testResult = {
          ...
          executionTime: Math.min(100 + (code.length * 0.1), 500), // Deterministic based on code size
          memoryUsed: Math.min(50 + (code.length * 0.05), 100), // Deterministic based on code size
          error: stderr
        };
        ...
        try {
          ...
          if (containerInfo) {
            ...
            console.log(`[DEBUG] Container removed successfully`);
          } else {
            console.log(`[DEBUG] Container already removed or doesn't exist`);
          }
        } catch (removeError) {
          // Handle specific Docker errors gracefully
          if (removeError.statusCode === 404) {
            console.log(`[DEBUG] Container already removed (404)`);
          } else if (removeError.statusCode === 409) {
            console.warn('Container removal conflict (409):', removeError.message);
          } else {
