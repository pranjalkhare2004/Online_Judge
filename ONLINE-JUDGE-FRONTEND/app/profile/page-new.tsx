    ...
    async function fetchProfileData() {
      ...
      try {
        ...
        if (userResponse.data.success) {
          ...
          setUser({
            id: userData.id,
            FullName: userData.FullName || authUser.FullName || 'User',
        ...
        if (submissionsResponse.data.success) {
          ...
          setSubmissions(submissionsData.map((sub: any) => ({
            ...
            problemName: sub.problemName || sub.problem?.title || 'Unknown Problem',
            problemId: sub.problemId || sub.problem?.id || '',
            verdict: sub.verdict || sub.status || 'Pending',
            language: sub.language || 'Unknown',
            submittedAt: sub.submittedAt || sub.createdAt || new Date().toISOString(),
            executionTime: sub.executionTime || sub.runtime,
            memoryUsed: sub.memoryUsed || sub.memory,
            contestName: sub.contestName || sub.contest?.name
          })))
        }
        ...
        if (contestsResponse.data.success) {
          ...
          if (Array.isArray(contestsData)) {
            setContests(contestsData.map((contest: any) => ({
              id: contest.id || contest._id || Math.random().toString(),
              title: contest.title || contest.name || 'Contest',
              rank: contest.rank || contest.position || 1,
              totalParticipants: contest.totalParticipants || contest.participants || 100,
              score: contest.score || contest.points || 0,
              maxScore: contest.maxScore || contest.totalPoints || 1000,
              date: contest.date || contest.startTime || new Date().toISOString(),
